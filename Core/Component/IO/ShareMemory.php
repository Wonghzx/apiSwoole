...
class ShareMemory
{
     ...
     */
    static public function getInstance($serializeType = self::SERIALIZE_TYPE_JSON, $file = null)
    {
    ...
    public function __construct($serializeType = self::SERIALIZE_TYPE_JSON, $file = null)
    {
        ...
        if ($file == null) {
            $file = Di::getInstance()->get(SHARE_MEMORY_FILE);
            if (empty($file)) {
                $file = Di::getInstance()->get(TEMP_DIRECTORY) . "/ShareMemory/shareMemory.men";
            }
        }
        $this->file = $file;
    }
